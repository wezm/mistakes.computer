...
(defn- validate-token [authorization]
  ...
  (and token (= authorization (string "Token " token))))

(defn- random-mistake [req ms]
  (util/seed-rng req)
  (mistakes/mistake-text ms (mistakes/pick ms)))

(defn- process-form [req text ms]
  (def mistake
    (if (or (nil? text) (empty? text))
      (random-mistake req ms)
      (string text " a mistake")))
  {:status 200
   :body (json/encode {:response_type "in_channel" :text mistake})
